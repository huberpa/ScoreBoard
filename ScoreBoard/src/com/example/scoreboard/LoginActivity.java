...
public class LoginActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// UML
		activity_login login = new activity_login();
						...
						bar.post(new Runnable() {
							...
							public void run() {
								...
								if (authorisierterNutzer == 1) {
									...
									korb.putStringArrayList("TTabelle",
											TTabelle);
									korb.putStringArrayList("BTabelle",
											BTabelle);
									korb.putStringArrayList("Groups", Groups);
									korb.putBoolean("tippenErlaubt",
											tippenErlaubt);
	...
	void getMoreInformation() {
		...
		if (Connect.verbindungAufbauen()) {
			...
			// Frage Tipps ab
			Connect.sendGetTipps(aktuellerSpieltag, Groups.get(aktuelleGruppe),
					Name);
